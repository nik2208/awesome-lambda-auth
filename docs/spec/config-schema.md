...
| RS-6 | Admin surface deployed with neither `admin.accessPolicy` nor `admin.bootstrapSecret`; **or** with an `admin.bootstrapSecret` shorter than 32 chars — beside a policy it is a password `POST <admin>/login` accepts for the empty email or `admin` with no bcrypt cost per guess, minting a root token (product clause, D8) | stderr warning, admin routes fully open; the secret is accepted at any length | `src/router/admin.router.ts:516-537`, `:558-563` |
...
| RS-13 | `stores.migration` configured without a source, a source without a pool id or region, or dual-read / a verifier on a driver that cannot hold the migration marker | n/a — product rule (the reference has no migration) | `internal/config/rules.go` `checkRS13Migration` |
| RS-14–RS-17 | reserved: the tools block (D9a) numbered its rules first | — | — |
| RS-18 | `admin.accessPolicy: first-user`, on **every** driver | grants whoever `listUsers(1, 0)` returns first, documented as "the first registered user" — true under the reference's monotonic ids only. The core's ids are 128 random bits and the listers order by id, so the console goes to the **lowest random id** and changes hands whenever a later registrant draws a lower one, through the public register route. Refused rather than repaired: a creation-ordered listing is a store seam the core would have to grow (product deviation `admin-first-user-policy-is-refused`) | `src/router/admin.router.ts:372-374`; core `security.go` `newID`, `admin.go` `evaluate`; `internal/store/dynamodb/users.go` (ordering) |
| RS-19 | `admin.enabled` under a session policy (anything but unset or `open`) with `cookies.sameSite: none` | **silent** — the admin router sits outside the CSRF chain (the SPA posts to `/login` with no CSRF header), the guard accepts the ordinary access-token cookie, and promote decodes its body with no `Content-Type` check, so `SameSite` is the only defence every admin write has against a cross-site form post | `src/router/admin.router.ts:543-611`, `:1030`; core `admin.go` "What is not wrapped around it", `admin_promote.go` |
