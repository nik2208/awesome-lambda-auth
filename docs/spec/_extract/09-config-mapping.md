...
| `onRegister` | function | unset → **`POST /register` not mounted** (`src/router/auth.router.ts:94-99`) — dev line (node-auth@e8af923): unset → **built-in handler, mounted by default** (`src/router/auth.router.ts:515-525`) | `registration.enabled` + policy, see §8 | |
...
| `routerOptions` | unset — `features.register` = `!!routerOptions?.onRegister` (`src/router/ui.router.ts:115`) — dev line (node-auth@e8af923): `routerOptions.onRegister` is the resolved `registerHandler` (`src/router/auth.router.ts:1793`; `src/router/ui.router.ts:121`) → `true` unless resource-server mode | n/a |
...
| `onRegister(data, config, options)` (`src/router/auth.router.ts:109`) | absent → `POST /register` **not mounted**; when present, receives raw body and must create+return the user — dev line (node-auth@e8af923): absent → built-in handler (`src/router/auth.router.ts:515-525`), mounted by default | `registration: {enabled: bool, fields: allowlist, passwordPolicy, autoVerify: bool}` with a built-in create-user handler; custom logic → `registration.webhook.url` (called before/after create) | built-in handler expressible; bespoke provisioning is code → webhook |
