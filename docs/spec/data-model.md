...
**User profile** — `_t: "user"`, `userId` S, `tenantId` S, `email` S (normalized, `service.go:58`), `passwordHash` S, `phoneNumber` S?, `firstName` S?, `lastName` S?, `role` S?, `isAdmin` BOOL, `loginProvider` S?, `isEmailVerified` BOOL, `require2FA` BOOL, `isTotpEnabled` BOOL, `totpSecret` S?, `resetHash`/`resetExp`, `magicHash`/`magicExp`, `verifyHash`/`verifyExp`, `echgHash`/`echgExp`, `pendingEmail` S?, `smsHash`/`smsExp`, `createdAt` S, `updatedAt` S. `~500 B` typical, `~1.5 KB` worst case.
...
**Role definition** — `_t: "role"`, `permissions` SS. Lives in the `ROLES` directory partition, not in one of its own; §1.4's correction gives the reason (`RoleLister.GetAllRoles` enumerates the set, and a per-role partition can only be enumerated by a Scan). The role name is **not** duplicated into an attribute: it is the tail of `SK` and `roleFromSK` is the only decoder, exactly as a mail template's id is. `permissions` is a String Set, not a List: it gives set semantics and read-free `ADD`/`DELETE`, so two concurrent grants both land with no read between them. Empty sets are illegal in DynamoDB, so a role with no permissions omits the attribute — which is also the state `DELETE` leaves when it removes the last member, so the two need no reconciling — and `GetPermissionsForRole` returns an empty slice for both.

**Metadata entry** — `_t: "meta"`, `userId` S, `value` (any JSON-shaped DynamoDB type), `updatedAt` S. The key is the tail of `SK`. *Corrected:* this said "via `attributevalue` marshalling of `any`", naming the AWS SDK's reflection-based marshaller. That module is not a dependency of this build and is not added for three methods — it marshals Go structs, which is the half nothing here needs, since every value that reaches these seams arrived as JSON. The codec is `anyvalue.go`, whose domain is the JSON value set plus the Go numeric kinds, `[]byte` and `time.Time`. Values with no representation (`chan`, `func`) are rejected at the store boundary, not silently dropped, and so is an integer beyond 2^53, because the codec decodes every number back as `float64` and a 64-bit identifier would not read back exactly. Capped at 64 KB per entry (`MaxMetadataValueBytes`), which with `Options.MaxMetadataKeysPerUser` on the count is what bounds the aggregate `GetMetadata` returns — see §6.3, corrected.

**Tenant** — `_t: "tenant"`, `name` S?, `isActive` BOOL, `config` M (any JSON-shaped value, always present, `{}` when empty), `createdAt` S, `updatedAt` S?. The id is the tail of `SK`. `isActive` is written even when false: a flag whose absence was indistinguishable from "nobody activated this tenant" would make `UpdateTenant`'s deactivation unobservable. Capped at 300 KB (`MaxTenantBytes`) because `Tenant.Config` is caller-supplied and unbounded in the interface.
...
- The 400 KB item limit binds only on metadata values and on the aggregate returned by `GetMetadata`, which is a `Query` and can therefore return megabytes across many items. *Corrected during implementation:* this asked for an aggregate byte cap per user, which cannot be enforced without reading the whole set on every `UpdateMetadata` — turning the one write that has no read into a read-modify-write, to defend a limit DynamoDB does not impose (the 400 KB limit binds per item, and a `Query` result is not an item). What actually needs bounding is the aggregate `GetMetadata` returns, and a **per-entry** cap of 64 KB (`MaxMetadataValueBytes`) together with a **key count** cap (`Options.MaxMetadataKeysPerUser`, default 1 000) bounds it, loudly, with nothing read before a write.
...
1. ~~**`UserMetadataStore` has no tenant id** (`store.go:85-87`).~~ **Closed by the store, not by upstream.** Metadata items moved out of `USER#<t>#<u>` and into `USERMETA#<u>`, following the linked-account item, which is keyed globally and carries no `tenantId` for exactly the same reason: `FindByProvider`, `ListForUser` and `Delete` carry no tenant either (§5). A user id is unique across tenants by construction, so a partition keyed on it alone is exactly as isolating as one keyed on both, and §3's rule is satisfied by the key rather than bent — there is no filter, and no query against this key space can reach a user the caller did not name. Neither the context-carried interim nor the upstream signature widening is needed. See §1.4's correction; the one consequence is that the `/me` bundle is two Queries rather than one.
...
6. **Unbounded list interfaces.** `GetUsersForTenant` (`[]string`), `GetAllTenants` (`[]Tenant`), `ListSessionsForUser` (`[]Session`), `ListForUser` (`[]OAuthLinkedAccount`) and the two template lists, `ListMailTemplates` (`[]MailTemplate`) and `ListUITranslations` (`[]UITranslation`), have no cursor parameter. The store must fully drain a paginated Query behind each. Proposed: a hard cap plus a typed `ErrResultTooLarge`, and an upstream request for cursor variants.
   ...
   **Answered for `ListSessionsForUser` (P1): 1000, configurable, refusing rather than truncating.** A user with four digits of live sessions is an incident to investigate, not a page to render, and a silent truncation would make `GET /sessions` lie about which devices are signed in — the one thing that screen exists to tell the truth about. **Answered for `ListForUser` the same way: 100, configurable, refusing.** A user with three digits of provider bindings is likewise an incident, and truncating would make `GET /linked-accounts` lie about which providers can sign the account in. **Answered for the two template lists the same way: 1 000, refusing** — there the number is an integrity check rather than a page size, because the directory holds the six reference ids, whatever the deployment adds to them and one entry per UI page (§1.6), so four digits of entries means something is writing them that should not be. **Answered for the remaining two, with different numbers, because they are different questions.** `GetAllTenants` is 1 000 (`Options.MaxTenants`): every tenant is one item in the single `TENANTS` partition, so a deployment with six figures of tenants has a hot partition (§6.2) long before it has a slow listing, and nobody reads four digits of rows off a screen. `GetUsersForTenant` is 10 000 (`Options.MaxUsersPerTenant`): that one is genuinely a membership list, and a tenant with ten thousand members is a customer rather than an incident. Both refuse rather than truncate. Two interfaces that landed with the same shape are answered the same way: `GetMetadata`/`ClearMetadata` at 1 000 keys per user (`Options.MaxMetadataKeysPerUser`, plus 64 KB per entry — see §6.3), and `GetAllRoles` at 1 000, where — as with the template directory — the number is an integrity check and not a page size.
7. ~~**`offset`-based pagination is not expressible in DynamoDB.**~~ **Answered, and by neither of the two options.** The admin surface takes `limit`/`offset` and returns a `total` the reference itself computes as a heuristic (`admin.router.ts:782`). Opaque cursors cannot express that arithmetic and would change the wire; reproducing the heuristic by over-reading is the route's job and needs no store support. What the store does instead is what the upstream author specified when they designed the index `AdminUserStore.ListUsers` queries: **`offset` is positional and is paid for by reading and discarding that many index entries** — index entries, never records, because the skip never resolves what it discards. That is bounded, since the route clamps `limit` to 100, and `Options.MaxPageWindow` (default 10 000) bounds `limit+offset` so that one query string cannot buy a Lambda timeout, answering `ErrPageWindowTooLarge` rather than truncating. The same primitive serves all four offset-paged interfaces: the three v0.8.0 admin listers, `APIKeyAdminStore.ListAll` and `WebhookAdminStore.ListWebhooks`.
...
**Three of them the core does not discover by type assertion.** `TemplateStore`, `SettingsStore` and `AuthCodeStore` are handed over explicitly — `auth.WithTemplateStore`, `auth.WithSettingsStore`, `IDPConfig.Codes` — so a signature that drifted out of shape would not fail to build and would not fail a store test either: the feature would simply stop being offered. That is why each of the three carries a compile-time assertion in its own file, why each is reached through an accessor the composition root asserts for structurally (`Templates()`, `Settings()`, `AuthCodes()`), and why `cmd/auth` pins the accessors as well as the interfaces.
