...
62 rows, covering 71 distinct method/code-path combinations (the four single-use token families are one parameterised block, §1.3). "Items" is the expected item count per invocation, not the RCU/WCU cost.
...
- `GET /oauth/{provider}` and `GET /oauth/{provider}/callback` answer the reference's "`<Provider>` OAuth not configured" 404, because `OAuthWiring.Service` needs the provider registry from the `oauth.*` configuration block and `internal/config/phases.go` still refuses that as a P4 domain. The four account-linking routes need no registry and work today.
