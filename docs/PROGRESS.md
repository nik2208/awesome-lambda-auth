...
Upstream: `v0.9.0` tagliato (M7); dopo il tag e atterrata U18, che ha finalmente dato dei chiamanti a `EventBus.Publish` — 16 punti Go per i 19 siti raggiungibili dei 26 della linea dev. In corso U12 (scheletro e guardia dell'admin, la PR di sicurezza di M8), U19 (formato webhook della reference, con la rimozione breaking di `X-Signature-SHA256`) e U20 (riscrittura SSE su `SseManager`, che rimuove `SseHub`). Prodotto: D3, D4 e X1 mergiati, pin a `v0.8.0`; quattro domini ancora gated (`ui`, `admin`, `tools`, `rateLimit`) e D5 li porta a tre. In corso D5 (rate limiting) e D6 (store admin).
