...
Upstream: `v0.9.0` tagliato (M7) — i 14 asset UI vendorizzati, `UIHandler` che li serve con SSR, catch-all e modalita headless, e il vocabolario `identity.*` col contesto di richiesta. In corso U18 (le 19 pubblicazioni raggiungibili delle 26) e U12 (scheletro e guardia dell'admin, la PR di sicurezza di M8). Prodotto: D3 e D4 mergiati, pin a `v0.8.0`; **quattro** domini ancora gated (`ui`, `admin`, `tools`, `rateLimit`) e D5 li porta a tre. In corso D5 (rate limiting), D6 (store admin) e X1 (Cognito, tornato indietro per una revisione).
