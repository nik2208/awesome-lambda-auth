...
Upstream: `v0.9.0` tagliato (M7); dopo il tag sono atterrate U18 (le pubblicazioni di eventi), U12 (scheletro e guardia dell'admin), U19 (formato webhook, breaking) e U20 (SSE su `SseManager`, breaking). In corso U13 (superficie admin in lettura) e U21 (facciata `AuthTools`, sul percorso critico). Prodotto: pinnato a `v0.9.0`; D3, D4, D5, D6 e X1 mergiati; **tre** domini ancora gated (`ui`, `admin`, `tools`). In corso D7 (UI ospitata), che ne toglie uno.
