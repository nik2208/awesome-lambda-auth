...
Upstream: M9 completo (U17-U25: vocabolario eventi, pubblicazione, formato webhook, SSE, facciata `AuthTools`, router tools con tutte e cinque le rotte) e M8 a una PR dalla fine — U12, U13, U14, U14b e U15 mergiate, resta U16. I tag `v0.10.0` (M8) e `v0.11.0` (M9) escono in quest'ordine appena U16 atterra; il merge di U25 e tenuto in sospeso apposta, perche mergiarlo prima metterebbe le rimozioni breaking di M9 dentro `v0.10.0`. Prodotto: pinnato a `v0.9.0`; D3, D4, D5, D6, D7, X1 e X4a mergiati; due domini ancora gated, `admin` (aspetta `v0.10.0`) e `tools` (aspetta `v0.11.0`).
...
### B9 — X4a osservabilita, allarmi e modello di costo (product · main · 2026-09-12)
Status: green
Landed: `37187d3` (PR #8, tre commit)
Gate: fmt ✓ vet ✓ race ✓ ddb-local ✓ in locale e su CI. build ✓ deploy – contract –
Deviations: nessuna (blocco infrastrutturale, nessuna superficie di filo tocca)
Decisions: **il budget nello stack non e un doppione di quello in console, e la ragione e la cosa piu importante del blocco.** Il budget esistente calcola il costo **crediti inclusi**: un credito applicato a un addebito e una riga negativa che lo azzera, quindi un account che brucia 40 USD al mese contro il credito promozionale gli mostra 0,00 USD e resta zitto per tutto il periodo in cui i soldi vengono spesi, scattando il mese in cui il credito finisce — cioe quando non serve piu. Quello nuovo imposta `IncludeCredit: false` e misura il **consumo lordo**, che e il numero contro cui e scritta la regola di stop a 120 USD cumulati. `ANNUALLY`, perche uno mensile si azzera prima di poter rispondere a una domanda cumulativa. Resta un avviso, mai un tetto. Nove allarmi scelti per l'incidente che intercettano e non per la metrica ovvia: `Duration` sul **massimo** e non sulla media, perche una singola invocazione su un downstream morto e invisibile in una media e una funzione che va in timeout fattura l'intero timeout; `ConcurrentExecutions` perche e l'unica metrica che si muove sia per un long-poll SSE che non chiude sia per un'invocazione ricorsiva, e si muove in minuti dove i segnali di spesa sono in ore o giorni; `IncomingBytes` sui log perche la retention limita lo storage e niente limita l'ingestione.
Upstream dance: nessun bump
Stack: nove allarmi, un topic SNS e una sottoscrizione (entrambi condizionati a un parametro vuoto), un budget, una rilevazione anomalie, e `ReservedConcurrentExecutions` (spento). **$0,00/mese a riposo e $0,00 con tutte le parti condizionate accese** — le nove metriche stanno dentro le dieci sempre gratuite di CloudWatch, e questo era un vincolo di progetto. In un account che ha gia speso le sue dieci: $0,90/mese. Costo di esercizio aggiunto: ~30 byte di correlation id su due righe per richiesta, circa **$0,03 per milione di richieste**. **Non ancora deployato.**
Notes: `docs/cost-model.md` esiste ora, ed e scritto perche D9c lo legga **prima** di essere scritto: una connessione SSE costa **0,024 USD per ora-connessione** a 512 MB, cioe **576 USD al giorno per mille ascoltatori**, mentre la lettura DynamoDB per poll all'heartbeat di 30 s e un **ottocentesimo** di quella cifra. La compatibilita di filo con `text/event-stream` costa ~3 ordini di grandezza sulle connessioni inattive rispetto a un WebSocket di API Gateway, ed e scritto come trade. Altro numero nuovo: un login e ~2 RRU e >=9 WCU — sei dei quali una `TransactWriteItems` a tariffa doppia piu due voci GSI1 — quindi **DynamoDB e circa tre quarti del costo di un login**, non bcrypt. Un allarme CloudWatch su `EstimatedCharges` e **impossibile** da qui: le metriche di fatturazione vivono solo in `us-east-1` e un allarme puo guardare solo la propria regione. La ritenzione dei log e resa strutturale da un test che legge il template, trova ogni funzione e fallisce se una non ha il suo log group con nome esatto, riferimento a `RetentionInDays` e `DependsOn` — verificato rompendolo, e fallisce anche sull'undicesimo allarme e su un account id, un ARN o un indirizzo committati.
Next: D8 quando esce `v0.10.0`, D9a-d quando esce `v0.11.0`
